<x-layout title="Labs, imaging and scans · Prompt Aid">
<div class="pa-pagehead"><div class="inner">
  <div class="pa-crumb"><a href="{{ url('/') }}">Home</a> <span style="color:#CFC8B8">/</span> Labs &amp; imaging</div>
  <h1>Labs, imaging and scans</h1><p class="pa-muted" style="font-size:15px;margin:6px 0 24px">Pathology, radiology and home draws with results into your record.</p>
  <div class="pa-tabs" style="gap:2px">
    ...
    <a class="pa-tab is-on" href="{{ route('labs.index') }}" style="border-bottom:0">Labs &amp; imaging</a>
...
<div class="pa-container" style="padding-top:32px;padding-bottom:80px">
  <form method="GET" style="margin-bottom:16px"><input class="pa-field" name="search" value="{{ request('search') }}" placeholder="Search labs and imaging centres…" style="max-width:360px" /></form>
  ...
  <div class="pa-grid" style="grid-template-columns:1fr">
      ...
      <div style="background:var(--pa-surface);padding:22px 24px;display:grid;grid-template-columns:64px minmax(0,1fr) auto;gap:20px;align-items:start">
        ...
        <div style="min-width:0">
          <div class="pa-row" style="flex-wrap:wrap;margin-bottom:5px">
            <a href="{{ route('labs.show', $provider) }}" style="font-size:18px;font-weight:900;letter-spacing:-.012em;color:var(--pa-ink)">{{ $provider->company_name }}</a>
        ...
        <div style="text-align:right;min-width:158px">
          ...
          <a class="pa-btn pa-btn-block" href="{{ route('labs.show', $provider) }}">View lab</a>
    ...
    @empty
      <p class="pa-muted">No labs listed yet.</p>
    @endforelse
  </div>
