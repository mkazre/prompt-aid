  ...
  <div class="pa-grid" style="grid-template-columns:repeat(auto-fill,minmax(230px,1fr))">
    @forelse ($products as $product)
      <div style="padding:20px;display:flex;flex-direction:column">
        <a href="{{ route('shop.show', $product) }}" class="pa-slot" style="height:130px;margin-bottom:14px;position:relative;font-size:30px;font-weight:900;letter-spacing:0;color:#CFC8B8">{{ mb_strtoupper(mb_substr($product->name, 0, 2)) }}
        ...
        <a href="{{ route('shop.show', $product) }}" style="font-size:14px;font-weight:700;line-height:1.35;margin-bottom:6px;color:var(--pa-ink)">{{ $product->name }}</a>
        <div style="font-size:12px;color:var(--pa-muted);margin-bottom:14px">{{ $product->pharmacy->name }}</div>
        <div class="pa-spread" style="margin-top:auto"><span class="pa-num" style="font-size:20px">R {{ number_format($product->price, 2) }}</span><a class="pa-btn-quiet pa-btn-sm" href="{{ route('pharmacies.show', $product->pharmacy) }}?product={{ $product->id }}">Add</a></div>
      </div>
    @empty
